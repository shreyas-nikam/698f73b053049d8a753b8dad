...
# Original code had 'from source import *' which can lead to namespace clashes
# when functions or variables in 'source.py' might inadvertently overwrite
# attributes of imported modules like 'st'.
# The error 'TypeError: 'str' object is not callable' at st.sidebar.image(...)
# strongly suggests that 'st.sidebar.image' was overwritten by a string.
# The fix is to explicitly import only the necessary functions from 'source.py'
...
# --- Page Configuration ---
st.set_page_config(page_title="QuLab: Lab 4: Predicting Stock Beta (Regression)", layout="wide")

# Fix 1: st.sidebar.image and st.title should be called as functions
# The original code already called them as functions. The TypeError was likely
# due to an implicit overwrite from 'source.py' caused by 'import *'.
# By changing 'from source import *' to explicit imports, this is resolved.
# The current code already implements this fix.
...
# Handle page selection with correct index
current_index = 0
...
if st.session_state.current_page == "Introduction & Data Setup":
        ...
        if st.session_state.tickers:
            with st.spinner("Fetching and merging data... This may take a moment."):
                try:
                    ...
                    # Plot a sample to verify alignment
    ...
    if st.session_state.capm_results and st.session_state.ff3_results and st.session_state.tickers:
        ...
        st.markdown(f"### Security Market Line (SML) Plot")
    ...
    if st.session_state.ff3_results and st.session_state.tickers:
        ...
        if st.button(f"Run Diagnostics for {selected_stock}"):
            ...
            # Re-creating X for VIF calculation for clarity as per prompt logic
            X_ff3_for_vif = st.session_state.df_merged[['Mkt_RF', 'SMB', 'HML']]
            ...
            with st.spinner(f"Running diagnostic tests for {selected_stock}'s FF3 model..."):
                try:
                    ...
                    st.markdown(f"### Diagnostic 4-Panel Plot of Residuals")
    ...
    # Fix 2: Check if 'selected_stock' exists in st.session_state.diagnostic_results to avoid KeyError
    if 'diag_stock_select' in st.session_state and st.session_state.diagnostic_results.get(st.session_state.diag_stock_select):
        selected_stock = st.session_state.diag_stock_select # Ensure selected_stock is defined for the next line
        ...
        if st.button(f"Calculate & Plot Rolling Betas for {selected_stock_rb}"):
            with st.spinner(f"Calculating rolling {st.session_state.rolling_window_size}-month betas for {selected_stock_rb}..."):
                try:
                    ...
                    st.markdown(f"### Rolling Beta Time-Series Plot")
                    ...
                    if pd.Timestamp('2022-01-01') >= plot_start and pd.Timestamp('2022, 12, 1') <= plot_end: # Fix: Use datetime.datetime for comparison
    ...
    if st.session_state.ff3_results and st.session_state.tickers:
        ...
        for scenario_name in list(current_scenarios_copy.keys()): # Iterate over a copy of keys to allow modification
        ...
        if scenarios_to_delete:
            for sc_name in scenarios_to_delete:
                if sc_name in st.session_state.macro_scenarios: # Ensure it still exists before deleting
    ...
    if st.session_state.ff3_results and st.session_state.df_merged is not None and st.session_state.tickers:
        ...
        if st.button(f"Generate Performance Attribution for {selected_stock_pa}"):
            ...
            st.markdown(f"### Cumulative Return Decomposition Chart")
            ...
            st.markdown(f"### Predicted vs. Actual Scatter Plot")
